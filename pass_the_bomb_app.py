...
APP_VERSION = "3.0 Simplified Timer"
...
# DEFAULT_BOMB_FUSE_OPTIONS removed
...
def format_timedelta(td):
    ...
    if total_seconds < 60 or not parts: # Show seconds if less than a minute or it's the only unit
...
def _serialize_state(state_dict):
    ...
    for key, value in state_dict.items():
        ...
        elif key == "history" and isinstance(value, list): # explosion_log removed
...
def _deserialize_state(json_data):
    ...
    for key, value in json_data.items():
        ...
        if key == "history" and isinstance(value, list): # explosion_log removed
            ...
            for record_str_dict in value:
                if isinstance(record_str_dict, dict):
                    ...
                    st.warning(f"Skipping non-dict record in '{key}' during deserialization: {record_str_dict}")
...
default_state_keys = {
    ...
    "game_end_time": None, "history": [], "oldest_ticket_days_to_beat": 0,
    "game_id": None # bomb_timer_end, explosion_log, bomb_fuse_setting removed
}
    ...
    with player_col1:
        with st.form("add_players_form", clear_on_submit=True):
            ...
            if add_player_submitted and name.strip():
                ...
                else: st.warning(f"{name.strip()} is already in the list.")
    with player_col2:
        if st.session_state.pending_players:
            ...
            for i, player_name in enumerate(list(st.session_state.pending_players)):
                ...
                if row_col2.button("❌", key=f"remove_player_{player_name}_{i}", help=f"Remove {player_name}"):
                    st.session_state.pending_players.pop(i)
                    st.rerun()
        else: st.markdown("_No players added yet for this game._")

    game_duration_label = st.selectbox(
        "Select game duration:", options=list(DEFAULT_GAME_DURATIONS.keys()), index=2) # Default to 1 hour
    # Bomb fuse selection removed
...
if st.session_state.game_started:
    ...
    # Gameplay Active
        ...
        can_pass = False
        pass_to_options = []
        if st.session_state.current_holder and st.session_state.players:
             pass_to_options = [p for p in st.session_state.players if p != st.session_state.current_holder]
             if pass_to_options: can_pass = True

        if not can_pass:
            st.error("Cannot pass the bomb: No available players to pass to, or current holder is invalid.")
        else:
            with st.form("pass_form"):
                ...
                default_ticket_date_val = datetime.now().date() - timedelta(days=max(0,st.session_state.oldest_ticket_days_to_beat) + 1)
                ...
                if submit_pass:
                    if not ticket_number.strip(): st.warning("⚠️ Please enter a ticket number.")
                    else:
                        days_old = (datetime.now().date() - ticket_date).days
                        if days_old < 0: st.error("Ticket creation date cannot be in the future!")
    ...
    with st.expander("📜 Bomb Pass History", expanded=True):
        if not st.session_state.history: st.caption("_No passes made yet._")
            ...
            for record in reversed(st.session_state.history):
                ...
                st.markdown(f"- `{record.get('from','N/A')}` ➡️ `{record.get('to','N/A')}` (Ticket: `{record.get('ticket','N/A')}` – **{record.get('days_old','N/A')}d old**) at {time_str}")
    # Fuse Explosion Log expander removed
...
with st.sidebar:
    ...
    else: st.caption("Start a new game or use a Game ID URL to load an existing one.")
    ...
    if st.session_state.game_started:
        ...
        else: st.caption("_No players in current game._")
        st.markdown("---")
    ...
    if st.button("🔄 Start New Setup / Restart App", type="primary"):
        ...
        st.toast("App reset. Setup a new game or load one via URL.", icon="🧹")
        st.rerun()
