...
APP_VERSION = "4.2 Google Drive Persistence"
LOGO_PATH = "asmpt_logo.png"
# GAME_STATES_DIR removed

DEFAULT_GAME_DURATIONS = { # No changes here
...
drive_service = None
DRIVE_FOLDER_ID = None # Will be loaded from secrets

# Function to initialize Google Drive service (call once)
@st.cache_resource # Cache the resource so it's initialized only once
def init_drive_service():
    ...
    if "google_drive_folder_id" not in st.secrets:
        st.error("Google Drive Folder ID (google_drive_folder_id) not found in Streamlit Secrets.")
        return None, None
...
# ---------- Helper Functions (format_timedelta, generate_game_id - no changes) ----------
...
# _serialize_state and _deserialize_state are still crucial for datetime objects with JSON
...
def load_game_state_from_backend(game_id):
    global drive_service, DRIVE_FOLDER_ID # Ensure we use the initialized ones
    if not drive_service or not DRIVE_FOLDER_ID or not game_id:
        # if game_id and not (drive_service and DRIVE_FOLDER_ID):
        #     st.info("Google Drive not configured; cannot load game state.")
        return None
    ...
    try:
        ...
        # else:
            # st.info(f"Game file {file_name} not found in Drive folder.") # Can be noisy
        return None
    except HttpError as e:
        if e.resp.status == 404: # File not found
            # st.info(f"Game file {file_name} not found in Drive folder (404).")
            pass
        else:
...
def save_game_state_to_backend(game_id, state):
    ...
    file_name = f"{game_id}.json"
    try:
        serializable_dict_state = _serialize_state(state.copy()) # Operate on a copy
        # Ensure oldest_ticket_days_to_beat is not saved if present from old states
...
# ---------- Page Config, Logo & Title, Manage Game ID, Initialise Session State (no major changes, uses new persistence) ----------
# These sections will remain structurally the same as in Version 3.6/3.5
# The key is that they call the new load_game_state_from_backend and save_game_state_to_backend
...
if not drive_service or not DRIVE_FOLDER_ID:
    st.error("Google Drive service is not initialized. Please check secrets configuration. Game persistence will fail.")
else:
    ...
    if "game_loaded_from_backend" not in st.session_state:
        ...
        if current_game_id_from_url:
            ...
            else: # Failed to load (file not found or error)
                st.warning(f"Could not load game: {current_game_id_from_url}. Starting new game setup.")
                if "game_id" in query_params: del query_params["game_id"] # Corrected way to remove
...
# ---------- Game Setup UI (no changes needed to its internal logic) ----------
    ...
    if len(st.session_state.pending_players) < 2: st.info("Add at least 2 players.")
        ...
        if st.button("✅ Start Game",type="primary",use_container_width=True):
            ...
            if drive_service and DRIVE_FOLDER_ID: # Only save if Drive is configured
                ...
                st.error("Drive service not available. Game state NOT saved.")
            st.balloons(); st.rerun()


# ---------- Game Interface UI (no changes needed to its internal logic, except for form key) ----------
if st.session_state.game_started:
    ...
    if time_left_game.total_seconds() <= 0: # Game Over
        ...
        if not can_pass: st.error("Cannot pass bomb: No valid players to pass to.")
            ...
            # Dynamic form key still recommended
            ...
            with st.form(dynamic_form_key):
                ...
                d_val = datetime.now().date() - timedelta(days=max(0, int(st.session_state.get("oldest_ticket_days_to_beat", 0))) + 1)
                ticket_date = st.date_input("Ticket creation date:", max_value=datetime.now().date(), value=d_val)
                submit_pass_button_pressed = st.form_submit_button("Pass This Bomb!") # Minimal version
...
# ---------- Sidebar Controls (no changes needed to its internal logic) ----------
with st.sidebar:
